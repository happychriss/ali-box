...
/* Power management registers (from CST816S datasheet / reference driver) */
...
static esp_err_t touch_cst816s_i2c_write(esp_lcd_touch_handle_t tp, uint16_t reg, const uint8_t *data, uint8_t len);
...
esp_err_t esp_lcd_touch_new_i2c_cst816s(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    /* NOTE: Reset is intentionally NOT performed here.
     * The application should own the reset/wake sequence because boards differ
     * (polarity/pulse width/boot time) and the app may also manage CST816S sleep.
     */
...
esp_err_t esp_lcd_touch_cst816s_enter_sleep(esp_lcd_touch_handle_t tp)
{
    ESP_RETURN_ON_FALSE(tp != NULL, ESP_ERR_INVALID_ARG, TAG, "Touch controller handle can't be NULL");
    const uint8_t v = CST816S_POWER_MODE_SLEEP;
    /* Best-effort: if bus is busy or device already asleep, don't hard-fail the app.
     * But do return the I2C status to the caller.
     */
    return touch_cst816s_i2c_write(tp, CST816S_REG_SLEEP_MODE, &v, 1);
}

esp_err_t esp_lcd_touch_cst816s_exit_sleep(esp_lcd_touch_handle_t tp)
{
    ESP_RETURN_ON_FALSE(tp != NULL, ESP_ERR_INVALID_ARG, TAG, "Touch controller handle can't be NULL");

    /* Many CST816S modules require a reset pulse to reliably wake after deep sleep.
     * We use the existing reset helper if RST pin is connected; otherwise just delay.
     */
    if (tp->config.rst_gpio_num != GPIO_NUM_NC) {
        ESP_RETURN_ON_ERROR(touch_cst816s_reset(tp), TAG, "Reset failed");
...
#ifndef CONFIG_ESP_LCD_TOUCH_CST816S_DISABLE_READ_ID
    /* Optionally re-check the chip responds (best-effort). */
    (void)touch_cst816s_read_id(tp);
#endif
